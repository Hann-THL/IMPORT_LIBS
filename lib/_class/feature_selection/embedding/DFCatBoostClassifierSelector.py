...
class DFCatBoostClassifierSelector(BaseEstimator, TransformerMixin):
    def __init__(self, columns=None, k=None, **kwargs):
        self.columns        = columns
        self.k              = k
        self.selector       = SelectFromModel(CatBoostClassifier(**kwargs))
    ...
    def transform(self, X):
        ...
        features = self.stat_df[self.stat_df[('support' if self.k is None else 'k_support')]].sort_values(by='importance', ascending=False)['feature'].values
